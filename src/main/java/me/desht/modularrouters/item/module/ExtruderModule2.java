...
public class ExtruderModule2 extends ModuleItem implements IRangedModule {
    ...
    public void addSettingsInformation(ItemStack itemstack, List<Component> list) {
        ...
        list.add(Component.literal(ChatFormatting.YELLOW.toString()).append(ClientUtil.xlate("modularrouters.itemText.extruder2.template")));
        ...
        if (list.size() == size) {
            ...
            list.set(list.size() - 1, asFormattable(tc)
                    .append(Component.literal(" " + ChatFormatting.AQUA + ChatFormatting.ITALIC))
